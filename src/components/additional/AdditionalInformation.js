...
const AdditionalInformation = () => {
    ...
    const fetchAdditionalInfo = async () => {
        setFetching(true);
        fetchData('/AdditionalInformation').then(response => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (editingInfo) {
            updateData(`/AdditionalInformation/${editingInfo.id}`, infoToSave).then(() => {
                setRefresh(!refresh);
            });
        } else {
            createData('/AdditionalInformation', infoToSave).then(() => {
                setRefresh(!refresh);
            });
        }
    };
    ...
    const confirmDeleteInfo = () => {
        deleteData(`/AdditionalInformation/${infoToDelete}`).then(() => {
            setRefresh(!refresh);
